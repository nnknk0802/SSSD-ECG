...
class Residual_block(nn.Module):
    def __init__(self, res_channels, skip_channels, diffusion_step_embed_dim_out, in_channels, attn_num_heads, attn_dropout, label_embed_dim=None):
        ...
        self.attn1 = TransformerBlock(dim=2 * self.res_channels, num_heads=attn_num_heads, dropout=attn_dropout)
        self.attn2 = TransformerBlock(dim=2 * self.res_channels, num_heads=attn_num_heads, dropout=attn_dropout)
...
class Residual_group(nn.Module):
    ...
    def forward(self, input_data):
        ...
        diffusion_step_embed = calc_diffusion_step_embedding(diffusion_steps, self.diffusion_step_embed_dim_in, device=noise.device)
...
class SSSD_ECG_Attn(nn.Module):
    def __init__(
        ...
        super(SSSD_ECG_Attn, self).__init__()
