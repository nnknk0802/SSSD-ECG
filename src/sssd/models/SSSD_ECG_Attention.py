...
class Residual_block(nn.Module):
    def __init__(self, res_channels, skip_channels,
        ...
        # Replace S4Layer with AttentionLayer
        self.Attn1 = AttentionLayer(
            features=2*self.res_channels,
            lmax=attention_lmax,
            num_heads=attention_num_heads,
            dropout=attention_dropout,
            layer_norm=attention_layernorm
        )

        self.conv_layer = Conv(self.res_channels, 2 * self.res_channels, kernel_size=3)

        self.Attn2 = AttentionLayer(
            features=2*self.res_channels,
            lmax=attention_lmax,
            num_heads=attention_num_heads,
            dropout=attention_dropout,
    ...
    def forward(self, input_data):
        ...
        h = self.conv_layer(h)
        h = self.Attn1(h.permute(2, 0, 1)).permute(1, 2, 0)  # (L, B, C) -> (B, C, L)
        ...
        h = self.Attn2(h.permute(2, 0, 1)).permute(1, 2, 0)  # (L, B, C) -> (B, C, L)
...
class SSSD_ECG_Attention(nn.Module):
    """
    SSSD-ECG model with Full Attention instead of S4.
    This model replaces S4 layers with standard Transformer-style multi-head self-attention.
    """
    ...
    def __init__(self, in_channels, res_channels, skip_channels, out_channels,
                 ...
                 label_embed_dim=128):
        super(SSSD_ECG_Attention, self).__init__()
